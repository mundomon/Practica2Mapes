...
NSString * Practica2Mapes$ModuleRequireFormat = @"(function(exports){"
...
@implementation Practica2MapesObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_practica2mapes);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Practica2Mapes global object
	NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];

	// Load the "Practica2Mapes" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_practica2mapes = [[Practica2MapesObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];

	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_practica2mapes];

	NSString *_practica2mapesNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _practica2mapesNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_practica2mapes valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Practica2Mapes$ModuleRequireFormat, dirname, filename, code];
...
- (TiModule *)loadJavascriptText:(NSString *)data fromFile:(NSString *)filename withContext:(KrollContext *)kroll
{
	...
	if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
		@throw [NSException exceptionWithName:@"org.practica2mapes.kroll"
