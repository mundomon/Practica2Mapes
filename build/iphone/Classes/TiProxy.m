...
//Rare exceptions to indicate a bug in the _practica2mapes code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_practica2mapes = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_practica2mapes]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.practica2mapes.module"
